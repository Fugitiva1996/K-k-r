...
s.bandas;
  ...
  s.sonido = s.loadSound('radius1.mp3');
  s.print('El sonido ha sido cargado');
    
}
  
  s.setup = function(){
    s.createCanvas(450, 200);
  // rectMode(CENTER);
  ...
  s.botonStop = s.createButton('Random');
  s.botonStop.mousePressed(s.aleatorio);
...
w.op = w.false
  ...
  w.preload = function() {
    ...
    w.song = w.loadSound("radius1.mp3");
    
  }
   
  w.setup = function(){
  w.createCanvas(450,350);
